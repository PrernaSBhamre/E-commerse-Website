        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {[1, 2, 3, 4].map((item) => (
            <div key={item} className="group cursor-pointer bg-white rounded-xl border border-gray-100 overflow-hidden">
              <div className="bg-gray-50 h-[260px] flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
        {products && products.length > 0 ? (
          products.map((product) => (
            <div key={product._id || product.id} className="group cursor-pointer bg-white rounded-xl border border-gray-100 hover:border-red-500/30 overflow-hidden hover:shadow-[0_8px_30px_rgb(220,38,38,0.15)] transition-all duration-500">
                  ...
                  <div className="flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          {product.flashSale?.originalPrice && product.flashSale.originalPrice > product.price && (
                              <span className="text-gray-400 line-through text-xs font-medium">{formatPrice(product.flashSale.originalPrice)}</span>
                          )}
                          {product.originalPrice && product.originalPrice > product.price && (
                              <span className="text-gray-400 line-through text-xs font-medium">{formatPrice(product.originalPrice)}</span>
                          )}
                      </div>
                       <div className="flex items-center gap-1.5">
                           <div className="flex text-yellow-500">
                              {[...Array(5)].map((_, i) => (
                                  <svg key={i} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={i < Math.floor((product.averageRating || 4) * 5) ? "currentColor" : "#E5E7EB"} className="w-3.5 h-3.5">
                                      <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clipRule="evenodd" />
                                  </svg>
                              ))}
                          </div>
                          <span className="text-gray-400 text-xs font-medium">⭐ {(product.averageRating || 4).toFixed(1)}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        <span className="text-orange-600 text-xs font-medium flex items-center gap-1">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-3 h-3">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a1.5 1.5 0 001.5 1.5h16.5a1.5 1.5 0 001.5-1.5V8.25a1.5 1.5 0 00-1.5-1.5H3.75a1.5 1.5 0 00-1.5 1.5v10.5z" />
                            <path strokeLinecap="round" strokeLinejoin="round" d="M3 7.5h18M3 12h18M3 16.5h18" />
                          </svg>
                          {product.monthlySales || 0} sold
                        </span>
                      </div>
                  </div>
          ...
          <div className="col-span-full text-center py-12">
            <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-2xl p-8 border-2 border-dashed border-red-200 max-w-md mx-auto">
              ...
              <h3 className="text-2xl font-bold text-gray-800 mb-2">No Best Sellers Yet</h3>
              <p className="text-gray-600 mb-6">There are currently no best selling products for this month. Check back soon!</p>
              <div className="flex justify-center gap-4 text-sm text-gray-500">
                <span className="flex items-center gap-1">📊 Updated Daily</span>
                <span className="flex items-center gap-1">🏆 Top Performers</span>
